...
if($tester && $_SERVER['HTTP_REFERER']!='https://tg.appenot.com/front/?tester') {
...
if(!$user){
	...
	if(isset($start_params['f'])){
		...
		if(!is_numeric($ref_id)) $ref_id=false;
	}
	...
	User::create($telegram_user_id,[
		'username'=>(isset($user_init['username']) && trim($user_init['username'])!='') ? trim($user_init['username']) : NULL,
		'first_name'=>(isset($user_init['first_name']) && trim($user_init['first_name'])!='') ? trim($user_init['first_name']) : NULL,
		'last_name'=>(isset($user_init['last_name']) && trim($user_init['last_name'])!='') ? trim($user_init['last_name']) : NULL,
		'is_bot'=>isset($user_init['is_bot']) ? $user_init['is_bot'] : 0,
		'language_code'=>isset($user_init['language_code']) ? $user_init['language_code'] : NULL,
		'is_premium'=>isset($user_init['is_premium']) ? $user_init['is_premium'] : 0,
		'platform'=>$platform,
		'version'=>$version_bot,
	],$ref_id,$user_target_source);
...
if($user['number_active_day']>0 && $endOfDay_2DayAgo>$user['last_get_day_reward']){
...
if($last_time_active && time()-60>$last_time_active) {
	...
	if(($now_time-$last_time_active)>$user_stati['passive_earn_time_seconds']) {
...
$evryday_box = Format::evryday_box($user['number_active_day'],$user['last_get_day_reward']);
...
echo json_encode([
	...
	'user'=>$user,
	'first_name'=>isset($user_init['first_name']) ? $user_init['first_name'] : "Аноним",
	'balance'=>$user['balance'],
	'tap_earn'=>$user_stati['tap_earn'],
	'tap_earn_coin_name'=>User::declension($user_stati['tap_earn'],"монета","монеты","монет"),
	'hour_earn'=>$user_stati['hour_earn'],
	'hour_earn_coin_name'=>User::declension($user_stati['hour_earn'],"монета","монеты","монет"),
	'current_hour_earn_coins'=>$current_hour_earn_coins,
	'current_hour_earn_name'=>User::declension($current_hour_earn_coins,"монета","монеты","монет"),
	...
	'user_lvl_with_star'=>$user['level'].' '.Format::get_lvl_stars($user['level']),
	'user_lvl_star'=>Format::get_lvl_stars($user['level']),
	'btn_share_friend'=>Format::btn_share_link('https://t.me/spin_racoon_bot/play?startapp=f_'.User::GetIdHash($telegram_user_id)),
	'friends_count'=>$user['friends_count'],
	'friend_earn'=>$user_stati['friend_earn'].' '.User::declension($user_stati['friend_earn'],"монета","монеты","монет"),
	'friend_link'=>'https://t.me/spin_racoon_bot/play?startapp=f_'.User::GetIdHash($telegram_user_id),
	'friend_bonus'=>isset($friend_bonus) ? $friend_bonus.' '.User::declension($friend_bonus,"монета","монеты","монет") : false,
